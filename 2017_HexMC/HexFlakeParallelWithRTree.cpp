...
typedef std::pair<Point_2, int> Value;
typedef index::rtree< Value, index::quadratic<2*256*256> > Centers;
	 					   ...
	 					   Polygon_2 &pgn_probe, int pgn_index)
{
	...
	centers.query(index::nearest(center, 6), std::back_inserter(result_s));
    BOOST_FOREACH(Value const& v, result_s)
    {
        if(v.second == pgn_index)
            continue;
        if(overlaps(lattice[v.second], pgn_probe))
        {
            flag = true;
            break;
        }
...
void Run1(std::vector<Polygon_2> &lattice, 
...
void Run2(std::vector<Polygon_2> &lattice, 
		int steps_count, int nx, int ny, double xmin, double xmax, double ymin, double ymax, int count_threads)
{
	RandomGenerator rnd;
	Polygon_2 pgn_probe;
	...
	for(int i=0; i < lattice.size(); i++)
	{
		centroid(lattice[i], p);
        centers.insert(std::make_pair(p, i));
	};
	// Максимальные смещения
	double max_translation = 0.1;
	double max_angle = 0.1;
	...
	omp_set_num_threads(count_threads);
	std::vector<Polygon_2> pgn_probes(count_threads);
    omp_lock_t rtreelock;
    omp_init_lock(&rtreelock);
	for(int i = 0; i < steps_count; i++)
	{
		cout << "Time step " << i << endl;
		auto bt = std::chrono::high_resolution_clock::now();
		accept_global = 0;
		//bool flag = false;
		#pragma omp parallel private(pgn_probe, accept_local, p, result_s) shared(pgn_probes, accept_global, rtreelock)
		{
			int size = lattice.size();
			int num = omp_get_thread_num();
			int low = num*size/count_threads;
			...
			for (int k = low; k < high; k++)
			{
                centroid(lattice[k], p);
                result_s.resize(0);
                centers.query(index::nearest(p, 1), std::back_inserter(result_s));
                Value v = result_s[0];
				// cout << "Polygon " << k << endl;
				// Двигаем полигон с индексом k пока он ни с кем не пересекается
				pgn_probes[num] = MovePolygon(lattice, v.first, k, max_translation, max_angle, rnd, xmin, xmax, ymin, ymax);
				/*
				do{
					pgn_probes[num] = MovePolygon(lattice, centers, k, max_translation, max_angle, rnd, xmin, xmax, ymin, ymax);
					#pragma omp barrier
				...
				*/
				#pragma omp barrier
				// Проверяем, что полигон pgn_probe ни с кем не пересекается (с учетом периодичности границ)
				if (CheckPolygonIntersect(lattice, centers, v.first, pgn_probes[num], k) == false)
				{
					lattice[k] = pgn_probes[num];
					boost::geometry::centroid(lattice[k], p);
                    omp_set_lock(&rtreelock);
                    centers.remove(v);
                    centers.insert(std::make_pair(p, k));
                    omp_unset_lock(&rtreelock);
					accept_local += 1;
				};
			};
			#pragma omp atomic
...
std::string SettingsToString(int nx, int ny, double xmin, double xmax, double ymin, double ymax)
{
	return std::to_string(nx) + " " +std::to_string(ny) + " " +std::to_string(xmin) + " " + 
		std::to_string(xmax) + " " + std::to_string(ymin) + " " + std::to_string(ymax); 
}

void SettingsFromString(std::string str, int& nx, int& ny, double& xmin, double& xmax, double& ymin, double& ymax)
{
...
int main(int argc, char* argv[])
{
	...
	Run2(lattice, steps_count, nx, ny, xmin, xmax, ymin, ymax, count_threads);
	cout << "Done!" << endl;
