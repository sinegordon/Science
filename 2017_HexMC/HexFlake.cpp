...
typedef std::pair<Point_2, int> TreeValue;
typedef index::rtree< TreeValue, index::linear<8> > RTree;
...
// Проверяем, что полигон pgn_probe, полученный из полигона с индексом pgn_index, никого не перекрывает
bool CheckPolygonIntersectTree(std::vector<Polygon_2> &lattice,
						   std::vector<Point_2> &centers,
						   RTree &centers_tree,
	 					   ...
	 					   Polygon_2 &pgn_probe, int pgn_index)
{
	...
	centroid(pgn_probe, p);
	result_s.resize(0);
	centers_tree.query(index::nearest(p, 10), std::back_inserter(result_s));
	BOOST_FOREACH(TreeValue const& v, result_s)
	{
		dist = comparable_distance(p, centers[v.second]);
		if( v.second == pgn_index || dist > 4)
			continue;
...
bool convex_overlaps(Polygon_2 pgn1, Polygon_2 pgn2)
{
		...
		int steps_count, int nx, int ny, double xmin, double xmax, double ymin, double ymax)
{
	...
	RTree centers_tree;
	std::vector<TreeValue> result_s(0);
	for(int i=0; i < lattice.size(); i++)
	{
		centroid(lattice[i], centers[i]);
		centers_tree.insert(std::make_pair(centers[i], i));
	};
	...
	for(int i = 0; i < steps_count; i++)
	{
		...
		for(int k = 0; k < lattice.size(); k++)
		{
			...
			/*
			if (CheckPolygonIntersect(lattice, centers, pgn_probe, k) == false)
			{
				lattice[k] = pgn_probe;
			...
			*/
			if (CheckPolygonIntersectTree(lattice, centers, centers_tree, result_s, pgn_probe, k) == false)
			{
				centers_tree.remove(std::make_pair(centers[k], k));
				lattice[k] = pgn_probe;
				boost::geometry::centroid(lattice[k], centers[k]);
				centers_tree.insert(std::make_pair(centers[k], k));
				accept += 1;
			};
