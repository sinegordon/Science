...
// Двигаем полигон с индексом pgn_index в решетке
void MovePolygon(std::vector<Polygon_2> &lattice, 
						...
						double xmin, double xmax, double ymin, double ymax)
{
	...
	if( rnd.NextDouble() > 0.5)
	{
	    ...
	    centers[pgn_index] = Point_2(x + dx, y + dy);
...
bool CheckPolygonIntersect(std::vector<Polygon_2> &lattice,
						   std::vector<Point_2> &centers, int pgn_index)
{
	bool flag = false;
	double dist = 0;
	for(int i = 0; i < pgn_index; i++)
	{
		dist = comparable_distance(centers[pgn_index], centers[i]);
		if ( dist > 4 )
		{
			continue;
		}
		...
		{
			flag = intersects(lattice[i], lattice[pgn_index]);
			if ( flag == true )
				break;
		}
	}
	if( flag == false)
	{
		for(int i = pgn_index+1; i < lattice.size(); i++)
		{
			dist = comparable_distance(centers[pgn_index], centers[i]);
			if ( dist > 4 )
			{
		...
		int steps_count, int nx, int ny, double xmin, double xmax, double ymin, double ymax)
{
	...
	for(int i = 0; i < steps_count; i++)
	{
		...
		for(int k = 0; k < lattice.size(); k++)
		{
			...
			pgn_old = lattice[k];
			center_old = centers[k];
			MovePolygon(lattice, centers, k, max_translation, max_angle, rnd, xmin, xmax, ymin, ymax);
