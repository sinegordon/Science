...
def render_2polygons_frame(frame, verts1, verts2, Ly=None):
    ...
    scene = fresnel.Scene(device)
    tid = frame.particles.typeid == 0
    ang = numpy.arctan2(frame.particles.orientation[tid,3], frame.particles.orientation[tid,0])*2
    g1 = fresnel.geometry.Prism(scene, vertices=verts1, position=frame.particles.position[tid,0:2], angle=ang, height=numpy.ones(ang.size)*0.5)
    g1.outline_width = 0.01
    g1.material = fresnel.material.Material(solid=1.0, color=blue)

    tid = frame.particles.typeid == 1
    ang = numpy.arctan2(frame.particles.orientation[tid,3], frame.particles.orientation[tid,0])*2
    g2 = fresnel.geometry.Prism(scene, vertices=verts2, position=frame.particles.position[tid,0:2], angle=ang, height=numpy.ones(ang.size)*0.5)
    g2.outline_width = 0.01
    g2.material = fresnel.material.Material(solid=1.0, color=blue)
