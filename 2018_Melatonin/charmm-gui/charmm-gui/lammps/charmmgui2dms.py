#!/usr/bin/env python
"""
charmmgui2dms.py: convert a charmm system to a Desmond dms/cms file or a LAMMPS data file
...
def write_dict(fp, name, dictt, prefix):
    fp.write("%s%s {\n"%(prefix, name))
    keylist = dictt.keys()
    keylist.sort()
    ...
    for key in keylist:
        if isinstance(dictt[key], dict):
            dict_key.append(key)
        elif isinstance(dictt[key], cmslist):
            cmslist_key.append(key)
        else:
            nondict_key.append(key)

    for key in nondict_key:
        fp.write("%s  %s\n"%(prefix, key))
    fp.write("%s  :::\n"%prefix)
    for key in nondict_key:
        fp.write("%s  %s\n"%(prefix, dictt[key]))
    ...
    for key in dict_key:
        write_dict(fp, key, dictt[key], prefix+"  ")

    for key in cmslist_key:
        dictt[key].write(fp, prefix+"  ")
...
class cmslist:
    ...
    def add_term(self, tt):
        assert len(tt.split()) == len(self.keys)
...
def dihe_compare(name1, name2):
    match = 0
    for k in range(0, 4):
        if name1[k].upper() == "X" or name1[k] == "*" or name2[k].upper() == "X" or name2[k] == "*":
            match += 1
        elif name1[k] == name2[k]:
            match += 1
    if match == 4:
    ...
    match = 0
    for k in range(0, 4):
        if namet[k].upper() == "X" or namet[k] == "*" or name2[k].upper() == "X" or name2[k] == "*":
            match += 1
        elif namet[k] == name2[k]:
            match += 1
    if match == 4:
        return True
    return False
...
class cms:
    def __init__(self, cmsfile):
        self.cmsfile = cmsfile
        self.colormap = {1:21, 6:145, 8:70, 7:43}
    
    def generate_cms(self, psf, crd, prm, temperature, boxsize, posref, atom_velocity):
        fpout = open(self.cmsfile, "w")
        version = {"s_m_m2io_version":"2.0.0"}
        write_dict(fpout, "", version,  "")
        syss = {}
        syss["s_m_title"] = "CHARMM-GUI_system"
        #syss["s_m_entry_id"] = "CHARMM-GUI system"
        ...
        for i in range(3):
            for j in range(3):
                syss["r_chorus_box_%s%s"%(aa[i], bb[j])] = "%f"%boxsize[i][j]
        syss["s_ffio_ct_type"] = "full_system"
        syss["i_m_ct_format" ] = "2"

        #ma = """r_m_x_coord r_m_y_coord r_m_z_coord i_m_residue_number 
        ma = """i_m_mmod_type r_m_x_coord r_m_y_coord r_m_z_coord i_m_residue_number 
                i_m_color s_m_chain_name s_m_pdb_residue_name s_m_pdb_atom_name i_m_atomic_number 
                i_m_formal_charge s_m_atom_name r_ffio_x_vel r_ffio_y_vel r_ffio_z_vel"""
        matom = cmslist("m_atom", ma.split())
        ...
        ma = "s_ffio_type i_ffio_resnr s_ffio_residue r_ffio_charge r_ffio_mass s_ffio_vdwtype"
        ffio_sites = cmslist("ffio_sites", ma.split())
        for i in range(0, psf.atomnum):
            anum = psf.atom[i].anum
            name = psf.atom[i].atomname
            atomtype = psf.atom[i].atomtype
            ...
            resid_t = resid + "_" + chainid
            if resid_t != resi_prev:
                resi_prev = resid_t
                resi += 1
            ...
            if anum in self.colormap:
                color = self.colormap[anum]
            else:
                color = 2

            formal_charge = 0
            #term = "%f %f %f %d %d %s %s %s %d %f %s %f %f %f"%(x, y, z, resi, color, chainid, resname,\
            term = "%d %f %f %f %d %d %s %s %s %d %f %s %f %f %f"%(mmodtype, x, y, z, resi, color, chainid, resname,\
                    name, anum, formal_charge, name, vx, vy, vz)
            matom.add_term(term)
            term = "atom %d %s %f %f %s"%(resi, resname, charge, mass, atomtype)
            ffio_sites.add_term(term)

        syss["m_atom"] = matom
        #atom done

        ma = "i_m_from i_m_to i_m_order"
        mbond = cmslist("m_bond", ma.split())
        for bond in psf.bond:
            ...
            if id1 > id2:
                ...
                id2 = temp
            order = 1
            term = "%d %d %d"%(id1, id2, order)
            mbond.add_term(term)
        syss["m_bond"] = mbond
        #bond done

        write_dict(fpout, "f_m_ct", syss,  "")

        syss["s_ffio_ct_type"] = "solute"

        ffio_ff = {}
        #ffio_ff["s_viparr_version"] = "None"
        ffio_ff["s_ffio_name"] = "CHARMM"
        ffio_ff["s_ffio_version"] = "36"
        #ffio_ff["s_viparr_command"]= "None"
        ffio_ff["s_date"] = datetime.datetime.now().strftime("%Y-%m-%d_%H:%M:%S")
        ffio_ff["s_user"] = getpass.getuser()
        ffio_ff["s_ffio_vdw_func"] = "LJ12_6_sig_epsilon"
        ffio_ff["s_ffio_comb_rule"] = "ARITHMETIC/GEOMETRIC"

        ffio_ff["ffio_sites"] = ffio_sites
        exclusion_list = set()

        if len(psf.bond) > 0:
            ma="i_ffio_ai i_ffio_aj s_ffio_funct r_ffio_c1 r_ffio_c2"
            ffio_bonds = cmslist("ffio_bonds", ma.split())
            bond_par_id = {}
            for i in range(0, len(prm.bond)):
                type1 = prm.bond[i].atom[0] + " " + prm.bond[i].atom[1]
                type2 = prm.bond[i].atom[1] + " " + prm.bond[i].atom[0]
                bond_par_id[type1] = i
                bond_par_id[type2] = i
            ...
            for bond in psf.bond:
                ...
                typee = type1 + " " + type2
                parid = bond_par_id[typee]
                bondpar = prm.bond[parid]
                term = "%d %d Harm %f %f"%(id1, id2, bondpar.b0, bondpar.kb)
                ffio_bonds.add_term(term)
                if id1 > id2:
                    exclusion_list.add((id2, id1))
                else:
                    exclusion_list.add((id1, id2))

            print "bonds done"
            ffio_ff["ffio_bonds"] = ffio_bonds


        if len(psf.angle) > 0:
            ma = "i_ffio_ai i_ffio_aj i_ffio_ak s_ffio_funct r_ffio_c1 r_ffio_c2"
            #s_ffio_funct = UB or Harm
            ffio_angles = cmslist("ffio_angles", ma.split())
            ...
            for angle in psf.angle:
                ...
                anglepar = prm.angle[prmid]

                ffio_angles.add_term("%d %d %d Harm %f %f"%(id1, id2, id3, anglepar.Theta0, anglepar.Ktheta))

                if anglepar.Kub != -1 and anglepar.S0 != -1:
                    angletype = "UB"
                    ffio_angles.add_term("%d %d %d UB %f %f"%(id1, id2, id3, anglepar.S0, anglepar.Kub))
                    
                if id1 > id3:
                    exclusion_list.add((id3, id1))
                else:
                    exclusion_list.add((id1, id3))

            ffio_ff["ffio_angles"] = ffio_angles
            print "angle done"

        no14pair = copy.deepcopy(exclusion_list)

        if len(psf.dihe) + len(psf.impr) > 0:
            ma = "i_ffio_ai i_ffio_aj i_ffio_ak i_ffio_al s_ffio_funct r_ffio_c0 r_ffio_c1 r_ffio_c2 r_ffio_c3 r_ffio_c4 r_ffio_c5 r_ffio_c6 r_ffio_c7"
            #s_ffio_funct = Improper_Harm or Proper_Trig
            #1 3 2 10 4 Improper_Harm 0 120 "ALLOW   PEP POL ARO" <> <> <> <> <> <>
            #16025 5582 2800 2801 5585 Proper_Trig 0 0.14 "ALLOW PRO" 0 0 0.14 0 0 0
            ffio_dihedrals = cmslist("ffio_dihedrals", ma.split())

            dihpair_psf = psf.get_uniq_dihedral_pairs()
            dihpair_psf1 = [i.upper().split() for i in dihpair_psf]

            prm_id = -1
            dihe2id = {}
            for i in prm.dihe:
                prm_id += 1
                dihe_prm_type = [k.upper() for k in i.names]
                for j in dihpair_psf1:
                    if dihe_compare(j, dihe_prm_type):
                        dihe_psf_type = " ".join(j)
                        if not dihe_psf_type in dihe2id:
                            dihe2id[dihe_psf_type] = [prm_id]
                        else:
                            dihe2id[dihe_psf_type].append(prm_id)

            for dihe in psf.dihe:
                types = [ psf.atom[j-1].atomtype.upper() for j in dihe ]
                type1 = " ".join(types)
                if not type1 in dihe2id:
                    print "can not find parameter for dihedral", type1
                    for j in dihe:
                ...
                if len(prmids) == 1:
                    ...
                    for j in prmids:
                        if not ("X" in prm.dihe[j].names or "*" in prm.dihe[j].names):
                            prmids1.append(j)
                assert len(prmids1) > 0

                for prmid in prmids1:
                    dihepar = prm.dihe[prmid]
                    term = "%d %d %d %d Proper_Trig %f %f %f %f %f %f %f %f"%(dihe[0], dihe[1], dihe[2], dihe[3],\
                            dihepar.delta, dihepar.fc[0], dihepar.fc[1], dihepar.fc[2], dihepar.fc[3],\
                            dihepar.fc[4], dihepar.fc[5], dihepar.fc[6])
                    ffio_dihedrals.add_term(term)

                if dihe[0] > dihe[3]:
                    exclusion_list.add((dihe[3], dihe[0]))
                else:
                    exclusion_list.add((dihe[0], dihe[3]))
            print "dihedral done"
            ...
            for prm_impr in prm.impr:
                prmid += 1
                #typee = " ".join(prm_impr.atoms)
                for psf_impr in psf_imprpair:
                    match = dihe_compare(prm_impr.atoms, psf_impr.split())
                    if match:
                        if not psf_impr in impr2id:
                            impr2id[psf_impr] = [prmid]
                        else:
                            impr2id[psf_impr].append(prmid)

            for psf_impr in psf.impr:
                typee = " ".join([psf.atom[i-1].atomtype for i in psf_impr])
                if not typee in impr2id:
                     raise Exception("no improper parameter for %s"%typee)
                ids = impr2id[typee]
                ids1 = []
                if len(ids) == 1:
                    ids1 = ids
                    ...
                    #if more than 1 match, remove wildcard match
                    for j in ids:
                        if prm.impr[j].atoms.count("X") == 0 and prm.impr[j].atoms.count("*") == 0:
                            ids1.append(j)
                assert len(ids1) > 0
                if len(ids1) > 1:
                    print type, "has more than 1 impr"
                for prmid in ids1:
                    imprpar = prm.impr[prmid]
                    term = "%d %d %d %d Improper_Harm %f %f <> <> <> <> <> <>"%(psf_impr[0], psf_impr[1], psf_impr[2],\
                            psf_impr[3], imprpar.psi0, imprpar.kpsi)
                    ffio_dihedrals.add_term(term)

            ffio_ff["ffio_dihedrals"] = ffio_dihedrals
            print "improper done"

        if len(exclusion_list) > 0:
            ffio_exclusions = cmslist("ffio_exclusions", ["i_ffio_ai", "i_ffio_aj"])
            for i in exclusion_list:
                term = "%d %d"%(i[0], i[1])
                ffio_exclusions.add_term(term)
            ffio_ff["ffio_exclusions"] = ffio_exclusions
            print "exclusion list done"

        #1-4 pairs
        ma = "i_ffio_ai i_ffio_aj s_ffio_funct r_ffio_c1 r_ffio_c2"
        ffio_pairs = cmslist("ffio_pairs", ma.split())
        pair14 = set()
        for dihe in psf.dihe:
            id1 = dihe[0]
            id2 = dihe[3]
        ...
        for p14 in pair14:
            id1 = p14[0]
            id2 = p14[1]
            term = "%d %d Coulomb 1 <>"%(id1, id2)
            ffio_pairs.add_term(term)
            type1 = psf.atom[id1-1].atomtype
            type2 = psf.atom[id2-1].atomtype
            e1 = prm.nonbond[prm.nonbond_dict[type1]].epsilon_14
            s1 = prm.nonbond[prm.nonbond_dict[type1]].sigma_14
            e2 = prm.nonbond[prm.nonbond_dict[type1]].epsilon_14
            s2 = prm.nonbond[prm.nonbond_dict[type1]].sigma_14
            if s1 < 0 or s2 < 0:
                if s1 < 0:
                ...
                assert sigma > 0
                assert epsilon > 0
                term = "%d %d LJ12_6_sig_epsilon %f %f"%(id1, id2, sigma, epsilon)
                ffio_pairs.add_term(term)
            else:
                term = "%d %d LJ 1 <>"%(id1, id2)
                ffio_pairs.add_term(term)
        if ffio_pairs.size() > 0:
            ffio_ff["ffio_pairs"] = ffio_pairs
            print "1-4 pair done"

        #vdw types
        ma = "s_ffio_name s_ffio_funct r_ffio_c1 r_ffio_c2"
        ffio_vdwtypes = cmslist("ffio_vdwtypes", ma.split())
        psfatomtypes = set([i.atomtype for i in psf.atom])
        for i in prm.nonbond:
            if i.name in psfatomtypes:
                term = "%s LJ12_6_sig_epsilon %f %f"%(i.name, i.sigma, i.epsilon)
                ffio_vdwtypes.add_term(term)
        ffio_ff["ffio_vdwtypes"] = ffio_vdwtypes
        ...
        if len(psf.cmap) > 0:
            ...
            for cmap in psf.cmap:
                ...
                cmap_types.append(" ".join(atomnames))

            cmapid = 1
            for cmap in prm.cmap:
                cmap_type = " ".join(cmap.atom).upper()
                if not cmap_type in cmap_types:
                    continue
                cmaplist = cmslist("ffio_cmap%d"%cmapid, "r_ffio_ai r_ffio_aj r_ffio_c1".split())
                cmap2id[cmap_type] = cmapid
                cmapid += 1
                #self.c.execute(""" CREATE TABLE cmap%d ('phi' float, 'psi' float, 'energy' float)""" %cmap.id)
                energyid = 0
                for phi in cmap.phi:
                    for psi in cmap.psi:
                        term = "%d %d %f"%(phi, psi, cmap.cmap[energyid])
                        cmaplist.add_term(term)
                        energyid += 1
                ffio_ff["ffio_cmap%d"%cmap.id] = cmaplist
            
            ma="i_ffio_ai i_ffio_aj i_ffio_ak i_ffio_al i_ffio_am i_ffio_an i_ffio_ao i_ffio_ap s_ffio_funct i_ffio_c1"
            ffio_torsion_torsion = cmslist("ffio_torsion_torsion", ma.split())
            for cmap in psf.cmap:
                atomnames = [psf.atom[i-1].atomtype.upper() for i in cmap]
                cmap_type = " ".join(atomnames) 
                cmapid = cmap2id[cmap_type]
                term = ["%d"%i for i in cmap]
                term = " ".join(term) + " cmap %d"%(cmapid)
                ffio_torsion_torsion.add_term(term)
            ffio_ff["ffio_torsion_torsion"] = ffio_torsion_torsion

            print "CMAP done"

       #nbfix 
        ma = "r_ffio_c1 r_ffio_c2 s_ffio_name1 s_ffio_name2 s_ffio_funct"
        ffio_vdwtypes_combined = cmslist("ffio_vdwtypes_combined", ma.split())
        psfatomtypes = set([i.atomtype for i in psf.atom])
        for nbfix in prm.nbfix:
            if nbfix.type[0] in psfatomtypes and nbfix.type[1] in psfatomtypes:
                term = "%f %f %s %s LJ12_6_sig_epsilon"%(nbfix.sigma, nbfix.epsilon_desmond, nbfix.type[0], nbfix.type[1])
                ffio_vdwtypes_combined.add_term(term)
        if ffio_vdwtypes_combined.size() > 0:
            ffio_ff["ffio_vdwtypes_combined"] = ffio_vdwtypes_combined
            print "nbfix done"

        #constraints for H atoms
        ma = "i_ffio_ai i_ffio_aj i_ffio_ak i_ffio_al i_ffio_am s_ffio_funct r_ffio_c1 r_ffio_c2 r_ffio_c3 r_ffio_c4 r_ffio_c5"
        ffio_constraints = cmslist("ffio_constraints", ma.split())

        #count how many hydrogen atoms each heavy atom is bonded
        nhydrogen_dict = {}
        for i in range(psf.atomnum):
            if psf.atom[i].anum > 1:
        ...
        for heavyatom_id in nhydrogen_dict:
            ...
            if psf.atom[heavyatom_id].resname.upper() == "TIP3" and nhydrogen == 2:
                ...
                #16662 16663 16664 0 0 HOH    104.519936      0.957200      0.957200      0.000000      0.000000 
                id1, id2 = nhydrogen_dict[heavyatom_id]
                term = "%d %d %d 0 0 HOH %f %f %f 0.0 0.0"%(heavyatom_id+1, id1+1, id2+1,\
                        prm.angle[angle_par_index].Theta0, bondlength[0], bondlength[1])
                ffio_constraints.add_term(term)
            else:
                atomids = ["%d"%(heavyatom_id+1)]
                constraint_par = []
                for i in range(nhydrogen):
                    atomids.append("%d"%(nhydrogen_dict[heavyatom_id][i]+1))
                    constraint_par.append("%f"%bondlength[i])
                for i in range(nhydrogen, 4):
                    atomids.append("0")
                for i in range(nhydrogen, 5):
                    constraint_par.append("0.0")
                term = "%s AH%d %s"%(" ".join(atomids), nhydrogen, " ".join(constraint_par))
                ffio_constraints.add_term(term)

        if ffio_constraints.size() > 0:
            ffio_ff["ffio_constraints"] = ffio_constraints
            print "constraints done"

        if len(posref) > 0:
            ma = "s_ffio_funct i_ffio_ai r_ffio_c1 r_ffio_c2 r_ffio_c3 r_ffio_t1 r_ffio_t2 r_ffio_t3"
            ffio_restraints = cmslist("ffio_restraints", ma.split())
            #1 harm  1    10.000000     10.000000     10.000000  -1.676273 -0.291309 19.761898
            #   2 harm  2    10.000000     10.000000     10.000000  -2.441078 0.63797 20.711216
            #      3 harm  6    10.000000     10.000000     10.000000  -0.273369 0.336773 19.636442
            #         4 harm  10    10.000000     10.000000     10.000000  -1.684376 -1.690048 20.27272

            for ref in posref:
                force = ref.posk
                for atom in ref.atom:
                    xyz = atom.coor
                    term = "harm %d %f %f %f %f %f %f"%(atom.atomid, force[0], force[1], force[2], xyz[0], xyz[1], xyz[2])
                    ffio_restraints.add_term(term)
            ffio_ff["ffio_restraints"] = ffio_restraints
            print "position restraints done"


        syss["ffio_ff"] = ffio_ff
        write_dict(fpout, "f_m_ct", syss,  "")
    ...
    def generate_dms(self, psf, crd, prm, temperature, boxsize, posref, atom_velocity):
        ...
        for angle in prm.angle:
            ...
            if pair in psf_angles or pair1 in psf_angles:
                ...
                if angle.Kub != -1 and angle.S0 != -1:
                    ...
                    ub_pair2id[typee] = bond_prm_number
        ...
        for i in prm.dihe:
            for dihe in dihpair_psf1:
                match = dihe_compare(i.names, dihe)
                if match:
...
def main():
    ...
    parser.add_argument('-cms', action="store", dest="outputcms",  required=False,\
            help="output cms file")
    ...
    if inputarg.crd:
        ...
        if inputpsf.atomnum != inputcrd.atomnum:
            raise Exception("number of atoms in psf and crd does not match: %d in psf %d in crd"%(inputpsf.atomnum, inputcrd.atomnum))    
    ...
    if posref and len(posref) != len(posk):
        raise Exception("number of positon restraint file and force constant set does not match")
    ...
    if outputcms != None:
        cmsfile = cms(outputcms)
        cmsfile.generate_cms(inputpsf, inputcrd, charmmprm, temperature, boxsize, posrestraint, velocity)
