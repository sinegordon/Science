...
def get_data(count):
    ...
    x_test = np.zeros((20, 1, 1, 100))
    y_train = np.zeros((count, nmax))
    y_test = np.zeros((20, nmax))
    p = np.zeros(3*nmax)
...
network = algorithms.Adadelta(
    [
        ...
        layers.Convolution((20, 1, 3)) > layers.BatchNorm() > layers.Relu(),
        layers.MaxPooling((1, 2)),
        layers.Convolution((50, 1, 3)) > layers.BatchNorm() > layers.Relu(),

        #layers.Convolution((64, 1, 10)) > layers.BatchNorm() > layers.Relu(),
